...
export function ParentListPage(props: ParentListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getParents({
        relations: ['user', 'student', 'conversation.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['relationship.contains', 'contact_number.contains', 'address.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/students/view/${record.student?.id}`}>
              {record.student?.performance_data}
            </Link>
          ),
        }
      : null,
    hasAccess('conversation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.conversation,
        }
      : null,
