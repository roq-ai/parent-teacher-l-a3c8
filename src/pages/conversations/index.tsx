...
type ColumnType = ColumnDef<ConversationInterface, unknown>;
...
export function ConversationListPage(props: ConversationListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getConversations({
        ...
        order: params.order,
        searchTermKeys: ['topic.contains', 'message.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ConversationInterface>>(
    () => `/conversations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ConversationInterface) => {
    if (hasAccess('conversation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/conversations/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/conversations/view/${record.id}`} passHref legacyBehavior>
                <Button
    ...
    <Box p={4} rounded="md" shadow="none">
      ...
      >
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="conversation" />
        </Flex>
